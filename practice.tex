...
X.  \ilc {k} is a function that defines an ITree program parameterized over an
argument of type \ilc X.  The constructed ITree first performs event \ilc e, and
uses its return value \ilc{(x: X)} to instantiate the continuation function
\ilc{(k x)}.
...
\ilc{(k x)} for every \ilc x of type \ilc X.
...
Here \ilc{(trigger e)} creates an ITree that performs an event \ilc{(e: E R)},
and halts with the return value of \ilc e.  \ilc{EXIT} halts the program with
unit value \ilc{tt}.

The \ilc{(m >>= k)} syntax \ilc{bind}s ITree program \ilc m to continuation
function \ilc k: Whenever ITree \ilc m halts at leaf \ilc{(Ret r)}, use the
return value \ilc r to instantiate the function \ilc k, and the program
continues with \ilc{(k r)}.  Another syntax for it is \ilc{(r <- m;; k r)}:
\begin{lstlisting}[style=customcoq]
...
In the \ilc{echo} definition, the return value of \ilc{putchar} is trivial and
ignored, so the syntax \ilc{(putchar c;; echo)} does not specify any bound
variable.
...
exhibits internal nondeterminism.  This can be represented as \ilc{Choice}
events:
\begin{lstlisting}[style=customcoq]
...
type as the original event, so it can replace the original \ilc{stdioE}.  This
is done by the \ilc{interp} function:
\begin{lstlisting}[style=customcoq]
...
instantiate the remainder program to interpret.  Therefore, \ilc{(interp
  (redirect input output) echo)} constructs a program that is equivalent with
\ilc{(redirect_echo input output)}.
