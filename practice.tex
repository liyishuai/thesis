...
Here the behavior after \ilc{recv} depends on the value received during.  Such
...
The three \ilc{echo} definitions are equivalent modulo syntactic sugars:
\begin{lstlisting}[style=customcoq]
...
In the \ilc{echo1} definition, the return value of \ilc{send} is trivial and
ignored, so the syntax \ilc{(send q;; echo)} does not specify any bound
variable.
...
exhibits internal nondeterminism, which can be represented as \ilc{Choice}
events:
\begin{lstlisting}[style=customcoq]
