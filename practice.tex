...
Here the behavior after \ilc{read} depends on the value actually read.  Such
...
shown in \autoref{fig:itrees}, \ilc{(itree E R)} represents a program with
interaction event type \ilc E and return type \ilc R.

An ITree program can be constructed in three ways: The \ilc{(Ret r)} constructor
corresponds to the trivial computation that halts and yields the value \ilc{r}.
The \ilc{(Tau t)} constructor corresponds to a silent step of computation, which
produces no effect and then continues as \ilc{t}.  Representing silent steps
explicitly with \ilc{Tau} allows us, for example, to represent diverging
computation without violating Coq's guardedness condition~\cite{coinduction}:
\begin{lstlisting}[style=customcoq]
  CoFixpoint spin {E R} : itree E R := Tau spin.
\end{lstlisting}

Here \ilc{spin} is an infinite program that never returns or performs any
events.  The usage of the \ilc{Tau} constructor will be further explained in the
next section.

The final and most interesting way to build an ITree is with the \ilc{(Vis X e
  k)} constructor.  Here \ilc{(e: E X)} is an event whose result is of type \ilc
X.  Function \ilc {k} defines an ITree program parameterized over an argument of
type \ilc X.  ITree \ilc{(Vis X e k)} first performs event \ilc e, and uses its
return value \ilc{(x: X)} to instantiate the continuation function \ilc{(k x)}.

These constructors build programs as tree structures, as the name ITree
suggests: \ilc{(Ret r)} constructs a leaf of value \ilc r; \ilc{(Tau m)} is a
node with single child \ilc m; \ilc{(Vis X e k)} has root \ilc e, and children
\ilc{(k x)} for every \ilc x of type \ilc X.  \autoref{fig:echo-itree} shows the
tree structure for the \ilc{echo} program.
...
The three definitions of echo are equivalent modulo syntactic sugars:
\begin{itemize}
\item The \ilc{getchar}, \ilc{putchar}, and \ilc{EXIT} used in \ilc{echo} and
  \ilc{echo2} are by themselves ITree programs:
\begin{lstlisting}[style=customcoq]
...
\end{lstlisting}
Here \ilc{(trigger e)} creates an ITree that performs a single event \ilc{(e: E
  R)}, and halts with the return value of \ilc e.  \ilc{EXIT} halts the program
with unit value \ilc{tt}.

\item The \ilc{(m >>= k)} syntax in \ilc{echo2} binds ITree program \ilc m to
  continuation function \ilc k: Whenever ITree \ilc m halts at leaf \ilc{(Ret
    r)}, use the return value \ilc r to instantiate the function \ilc k, and the
  program continues with \ilc{(k r)}:
\begin{lstlisting}[style=customcoq]
...
\item The \ilc{echo} definition, syntax \ilc{(c <- getchar;; ...)} binds the
  return value of \ilc{getchar} as variable \ilc c in the continuation.  The
  return value of \ilc{putchar} is trivial and ignored, so the syntax
  \ilc{(putchar c;; echo)} does not specify any bound variable:
\begin{lstlisting}[style=customcoq]
...
The network model captures TCP's feature of maintaining the order within each
...
\item Given a non-empty list, \ilc{pick_one} might return \ilc{(Some p)},
  meaning the wire can emit that packet; or it might return \ilc{None}, meaning
