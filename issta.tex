...
let check (trace  : stream http_message,
  ...
  \caption{Ad hoc tester for \http conditional requests, for demonstrating how
    ...
    to.
    \sz{Where is this figure mentioned in the text?  We should refer to
      it when describing the ad hoc testing logic in the text.} \bcp{Some
      questions readers might have: What is ``stream''? Why doesn't it
      distinguish between messages from the server and messages to the
      server? What is ``::''? What is ``strong\_match'' and
      ``weak\_match''?  What are ``is'' and ``is\_not'' for? Don't servers
      communicate with many clients? (The data structure here looks like a
      trace of interactions with just one client...)  \lys{Strong match and weak
        match are HTTP jargons.  ``is'' and ``is not'' are part of the ad hoc
        tester design.  Do we need to explain them thoroughly?}}  \bcp{Also, a
      more semantic question: Must the etag
      returned for a given value be globally unique?  Or can the server
      re-use an etag if the content is the same?  E.g., If I PUT ``X'', then do
      GET and
      get back etag ``eX'', then do PUT ``Y'', then do PUT ``X'' again and
      finally do GET, am I allowed to get back ``eX'' at the end?
      \lys{Sadly yes, but luckily nobody implemented servers in that way---they
        all used timestamps.  Our spec is more strict than RFC, but only rejects
        nonexisting implementations.}}
    %% \bcp{Should t' be trace'??}
  }
