...
styles, according to \citep{anand2013orchestrated}: (1) {\em Process-oriented}
...
navigate~\citep{anand2013orchestrated}.  Here we focus on techniques that have
...
Language of Temporal Ordering Specification (LOTOS)~\citep{Bolognesi1987} is the
...
Using a formal language strongly insired by LOTOS, \citet{torxakis-dropbox}
implemented a test generation tool for symbolic transition systems called
TorXakis, which has been used for testing Dropbox~\citep{torxakis-dropbox}.
...
\citet{netsem} have developed rigorous specifications for transport-layer
...
\citet{testing-dropbox} have introduced ``conjectured events'' to represent
uploading and downloading events that nodes may perform at any time invisibly.

\citet{pkt-dyn} symbolised the time elapsed to transmit packets from one end
