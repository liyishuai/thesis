...
\begin{definition}[Server transitions]
  Upon request $q$, server model $s$ can step to $s'$ yielding response $a$
  (written ``$\behaves s{(q,a)}s'$~'') if and only if the response and the post
  model can be computed by the $\stepServer$ function:

\begin{align*}
  &\behaves s{(q,a)}s'\quad\triangleq\quad\exists c,\stepServer(q,c)(s)=(a,s')\\
...
\begin{definition}[Trace validity in QAC]
  ...
  \item A server model can produce a non-empty trace if it can produce the head
    ...
    trace:
    \[\behaves s {t+(q,a)} s_2\quad\triangleq\quad\exists s_1,\behaves s t s_1\wedge\behaves {s_1}{(q,a)}s_2\]
  \end{enumerate}
\end{definition}
