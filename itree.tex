...
Instead of binding a program to a continuation, ITree binds single events, as
shown in \autoref{fig:itrees}.  I use \ilc{(Impure e k)} to replace \ilc{(Bind
(Trigger e) k)} representations in \ilc{itreeM}.  A \ilc{Pure} computation
cannot be bound to a continuation, and must be the leaf of an
ITree.\footnote{For readability, the ``practical'' ITree definition is a
...
\lys{Under construction:  End this section by embedding QAC into ITrees.
Move interpretation to next section, which handles internal nondeterminism.
After that, talk about external nondeterminism and network composition.}
