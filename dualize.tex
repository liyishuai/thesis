...
For simplicity, I demonstrate the computations with an abstract machine model
featuring random-access memory (RAM) and arithmetic operations of integers.  To
handle more complex data types like HTTP, we can extend the abstraction to
support those types, or reduce the computation from other types to integers.
...
\end{definition}
To encode specifications for protocols like CMP-RST, I introduce a simple
language $\Prog$ in the QAC family, which supports arithmetic operations and
memory access:
\[\begin{array}{lrll}
